...
### 3.3.1 적용된 다크 팔레트 (outo-models 서버)

v0.5.10+ 부터 `prefers-color-scheme: dark` 매체쿼리에 한해 §3.3 의 토큰이 자동으로 덮어써진다. `<meta name="color-scheme" content="light dark">` 가 같이 들어가 네이티브 컨트롤/스크롤바도 적응한다. 라이트 값은 *바이트 단위로 동일* — 매체쿼리 안에서만 재정의한다.

| 토큰 | 라이트 | 다크 | 비고 |
| --- | --- | --- | --- |
| `--tile-bg` | `#FAFCFF` | `#0E1420` | 다크는 거의-검정, 약간 푸른 틴트 |
| `--text` | `#000000` | `#E8EEF7` | 다크는 거의-흰색, 푸른 틴트 |
| `--text-sub` | `#3E4D5F` | `#9DA9BC` | 라이트 그레이-블루, `--tile-bg` 대비 > 7:1 |
| `--color-main` | `#007BFF` | `#4D9DFF` | 다크는 한 단계 밝게 — 다크 배경에서 4.5:1 유지 |
| `--color-inactive` | `#94989E` | `#6E7681` | 다크 배경에서 보이는 톤 |
| `--el-hover` | `#005BDD` | `#2F7FFF` | 채워진 요소 호버는 다크에서도 한 단계 더 어두운 톤 단방향 유지 (§3.4) |
| `--el-active` | `#0026A3` | `#3D5DDB` | 액티브/포커스 — 다크에서 채도 높임 |
| `--blp-light-dark-blue` | `#DBE3FF` | `#1A2940` | 비어있는 요소 포커스 시 *내부 변화 필수* — 다크 paper-blue |
| `--blp-paper-blue` | `#EEF5FC` | `#15233A` | 칩 배경 다크 톤 |
| `--blp-light-blue` | `#DBEDFF` | `#1F3050` | 칩 보더 다크 톤 |
| `--blp-paper-blue-2` | `#F4F8FC` | `#131C2E` | 더 미세한 톤 차이 |
| `--blp-soft-blue` | `#7FBCFF` | `#7DA9C7` | placeholder 다크 톤 |

> **§3.4 단방향 흐름 규칙은 다크 모드에서도 유지된다.** 호버/액티브 시 라이트↔다크 swap 이 일어나지 않는다 — 한 톤 어두워지거나 한 톤 밝아지는 단방향만.
>
> **자동 토글만 지원.** 사용자가 수동으로 라이트/다크를 고르는 토글은 의도적으로 두지 않는다 — OS 환경 설정을 따르며, 서버는 OS 시그널만 반영한다 (요청에 명시).
>
> **콘텐츠는 자유.** 코드 블록(`<pre>`) 의 배경·전경은 디자인 언어 밖 — 다크 모드에서 다크 page 위 경계가 흐려지지 않도록 다크 모드 한정 `border-color: var(--blp-light-blue)` 를 chrome 책임 범위에서 한 줄 추가했다. 본문 컬러는 손대지 않았다.
>
> **레포 액센트.** `color-mix(in srgb, {{ repo.color }} 14%, var(--tile-bg))` 패턴은 토큰만 참조하므로 자동으로 다크에 적응한다 — 별도 손 안 댐.
