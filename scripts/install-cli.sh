...
# CLI containers drive the host's podman through the API socket. Enable the
# rootless socket for the invoking user (rootful installs already have
# /run/podman/podman.sock via the system service). Linger keeps the socket
# alive across logouts. Best-effort: older systemd or missing user sessions
...
# The reset gate reads OUTO_DESTRUCTIVE; pass it through when set.
if [[ -n "\${OUTO_DESTRUCTIVE:-}" ]]; then
    args+=(-e OUTO_DESTRUCTIVE)
fi
...
# Same embedding convention for the low-port sysctl script (rootless hosts
# need it to bind 80/443). Drift-tested against the repo file as well.
low_ports_dest="/usr/local/share/outo-models/enable-low-ports.sh"
cat > "${low_ports_dest}" <<'OUTO_LOW_PORTS_HEREDOC_END'
#!/usr/bin/env bash
...
OUTO_LOW_PORTS_HEREDOC_END

chmod 0755 "${low_ports_dest}"

echo "[done] installed ${dest} (default image: ${image})"
echo "       installed ${firewall_dest} (host firewall script)"
echo "       installed ${low_ports_dest} (host low-port sysctl script)"
